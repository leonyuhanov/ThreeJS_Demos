...
import threeToroid from './BoilerPlate/threeToroid.js';					//Toroid
...
{	
	...
	{
		objectTape.push(new threeToroid());
		objectTape[objectCounter].init(scene, 0);
		objectTape[objectCounter].sliceOpacity = 0.2;
		objectTape[objectCounter].trailsPerSlice = 10;
		objectTape[objectCounter].showPoints = 0;
		objectTape[objectCounter].slices = 128;
		objectTape[objectCounter].type = 0;
		objectTape[objectCounter].trailPointSpacing = 0.003;
...
function animate() 
{
	...
	var colourIncrement = 1, subColourIncrement=5, motionSpeed = 0.02;
	var rotationSpeed = [0,0,0];	//change for x,y,z to roatat aorund the axis.
	
	for(localObjectCounter=0; localObjectCounter<objectTape.length; localObjectCounter++)
	{
		objectTape[localObjectCounter].animate( colourIncrement, subColourIncrement, motionSpeed, rotationSpeed ) ;
		//call below to change the orbits size, width and height
		//objectTape[localObjectCounter].updatePath(ORBITSCALE, WIDTHSCALE, HEIGTSCALE);
