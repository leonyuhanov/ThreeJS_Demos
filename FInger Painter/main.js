...
function setUp()
{
	...
	MIDISTore.addItem(176,5,"xy1", MIDISTore.getValueRounded("radius"));
	MIDISTore.setValue("xy1", 0);
	
	MIDISTore.addItem(176,40,"colourDecay", 100);
	MIDISTore.setValue("colourDecay", 0);
...
{
	if(MIDISTore.hasChanged("xy1"))
	{
		var xyArray = MIDISTore.getValueArray("xy1");
		if(xyArray.length!=0)
		{
			
			innerCIndex = cIndex;
			for(var cCount=0; cCount<5; cCount++)
			{
				cObjectOne.getColour(innerCIndex%cObjectOne._bandWidth);
				innerCIndex += MIDISTore.getValue('subColourIncrement');
				pixelMap.drawCircle(Math.round(xyArray[1]*MIDISTore.getValue("radius")), Math.round(xyArray[0]*MIDISTore.getValue("radius")), cCount, cObjectOne._currentColour, 1);
			}
...
{	
	...
	{
		recalculatePixelMap();
		MIDISTore.changeScaleToValue("xy1", MIDISTore.hasChanged("radius"));
	}		
}
